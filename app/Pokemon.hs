...
decideQuemVaiPrimeiro :: PokemonBattle -> PokemonBattle -> Bool
decideQuemVaiPrimeiro pok1 pok2 = spd (pkmn pok1) >= spd (pkmn pok2)
...
coletaItens :: String -> IO (Either String (Maybe Item))
coletaItens nomeItem = do 
	csvData <- BL.readFile "./data/itens.csv"
	let decoded = decode HasHeader csvData :: Either String (Vector Item)
	case decoded of
		Left err -> return $ Left err
		Right itens -> return $ Right (findItemByName nomeItem itens)
